...
#ifdef MY_ABC_HERE
static const char *gszInittab[]={
	"::sysinit:/etc/rc",
	"::respawn:/sbin/getty -n -l /etc/autologin.sh 115200 console",
	NULL,
};

static int gInitEntry = 0;
static int ReadInittab(char **tokens, int ntokens) {
	int i = 0;
	char szBuf[1024];
	char *ptr= NULL;

	if (NULL == gszInittab[gInitEntry]) {
		return -1;
	}

	memset(tokens, 0, sizeof(tokens[0]) * ntokens);
	snprintf(szBuf, sizeof(szBuf), "%s", gszInittab[gInitEntry++]);
	ptr = szBuf;
	for (i = 0; i < ntokens; i++) {
		tokens[i] = strsep(&ptr, ":");
	}
...
//TODO: VC_1 instead of ""? "" is console -> ctty problems -> angry users
 ...
 bad_entry:
#ifdef MY_ABC_HERE
		message(L_LOG | L_CONSOLE, "Bad inittab entry at line %d", gInitEntry);
	}
#else
...
#ifdef MY_ABC_HERE
#define MAX_MNTENT	128
#include <mntent.h>
#include <sys/mount.h>
/*
 *	Spawn an external program.
 */
static void umount_all_fs(void) {
	FILE *mountTable;
	struct mntent *mountEntry;
	char *rgszMnt[MAX_MNTENT];
	int idx = 0;

	if (0 == (mountTable = setmntent("/proc/mounts", "r"))) {
		message(L_LOG | L_CONSOLE, "Get mount entry failed");
		return;
	}
	bzero(rgszMnt, sizeof(rgszMnt[0]) * MAX_MNTENT);
	while (0 != (mountEntry = getmntent(mountTable))) {
		if (strcmp(mountEntry->mnt_dir, "/proc") &&
		    strcmp(mountEntry->mnt_type, "swap") &&
		    ('/' == mountEntry->mnt_dir[0])) {
			if (idx == MAX_MNTENT) {
				for (idx = 0; idx < MAX_MNTENT && rgszMnt[idx]; idx++) {
					message(L_CONSOLE, "Unmounting %s ...", rgszMnt[idx]);
					if (umount(rgszMnt[idx])) {
						message(L_LOG | L_CONSOLE, "Failed to umount %s, %d", rgszMnt[idx], errno);
					}
					free(rgszMnt[idx]);
					rgszMnt[idx] = NULL;
				}
			...
			if (idx < MAX_MNTENT) {
				rgszMnt[idx++] = strdup(mountEntry->mnt_dir);
			} else {
				message(L_CONSOLE, "Unmounting %s ...", mountEntry->mnt_dir);
				if (umount(mountEntry->mnt_dir)) {
					message(L_LOG | L_CONSOLE, "Failed to umount %s, %d", mountEntry->mnt_dir, errno);
				}
			}
		}
	}
	endmntent(mountTable);
	while((0 < idx) && (idx <= MAX_MNTENT)) {
		idx--;
		message(L_CONSOLE, "Unmounting %s ...", rgszMnt[idx]);
		if (umount(rgszMnt[idx])) {
			message(L_LOG | L_CONSOLE, "Failed to umount %s, %d", rgszMnt[idx], errno);
	...
	message(L_CONSOLE, "All file systems unmounted.");
}
#endif
...
#ifdef MY_ABC_HERE
	message(L_CONSOLE | L_LOG, "Unmounting all file systems.");
	sleep(2);
	umount_all_fs();
	sleep(3);
#endif
...
#endif
#ifdef MY_ABC_HERE 
	if (NULL != strstr(applet_name, "linuxrc")) {
		if (0 == access("/linuxrc.syno", F_OK)) {
			int ret;
			ret = system("/bin/ash /linuxrc.syno > /dev/console 2>&1");
			umount("/mnt");
			umount("/tmpRoot");
			umount("/proc");
			umount("/sys");
			if (0 == WEXITSTATUS(ret)) {
				message(L_CONSOLE | L_LOG, "linuxrc.syno executed successfully.");
				return 0;
			} else {
				message(L_CONSOLE | L_LOG, "linuxrc.syno failed on %d", ret / 256);
			}
		} else {
			message(L_CONSOLE | L_LOG, "linuxrc.syno access fail on %d", errno);
		}
		system("/bin/rm -f /etc && mv /etc.defaults /etc");
	}
#endif
