...
int date_main(int argc UNUSED_PARAM, char **argv)
{
 ...
 format_utc:
			...
			if (ENABLE_LOCALE_SUPPORT)
				setlocale(LC_TIME, "C");
#ifdef MY_ABC_HERE
			strcpy(fmt_dt2str, (opt & OPT_UTC)? "%a, %e %b %Y %H:%M:%S %Z" : "%Y %H:%M:%S %z");
		} else { /* default case */
			strcpy(fmt_dt2str, "%e %H:%M:%S %Z %Y");
#else
...
#ifdef MY_ABC_HERE
		if (0 == strcmp(fmt_dt2str, "%e %H:%M:%S %Z %Y")) {
			if (0 <= tm_time.tm_wday && 7 > tm_time.tm_wday) {
				printf("%s ", grgszWeekDay[tm_time.tm_wday]);
			}
			if (0 <= tm_time.tm_mon && 12 > tm_time.tm_mon) {
				printf("%s ", grgszMonth[tm_time.tm_mon]);
			}
		} else if (0 == strcmp(fmt_dt2str, "%Y %H:%M:%S %z")) {
			if (0 <= tm_time.tm_wday && 7 > tm_time.tm_wday) {
				printf("%s, ", grgszWeekDay[tm_time.tm_wday]);
			}
			if (0 <= tm_time.tm_mon && 12 > tm_time.tm_mon) {
				printf("%d %s ", tm_time.tm_mday, grgszMonth[tm_time.tm_mon]);
			}
		}
#endif
	}
