...
#ifdef SYNO_PPC_QORIQ
#include <synosdk/external.h>
...
#ifdef MY_ABC_HERE
//Be consistent with SZF_ENC_SHARE_LOCK at dsm/include/webman.h
#define SZF_ENC_SHARE_LOCK "/tmp/enc.share.lock" 
//Be consistent with SZ_SHUTDOWN_CMD at dsm/modules/ControlPanel/modules/Power/power.cpp
#define SZ_SHUTDOWN_CMD "/sbin/poweroff"
#endif
...
static void crondlog(const char *ctl, ...)
{
	...
	if (level >= (int)LogLevel) {
...
#ifdef MY_ABC_HERE
			if (logfd >= 0) {
				xdup2(logfd, STDOUT_FILENO);
				xmove_fd(logfd, STDERR_FILENO);

				// append time prefix (date_buf) into syslog if it's logged into file
				time(&tm);
				localtime_r(&tm, &tm_time);
				strftime(date_buf, sizeof(date_buf), "%b %d %T ", &tm_time);
			}
#else
			if (logfd >= 0)
				xmove_fd(logfd, STDERR_FILENO);
		...
		if (level > 8) {
			...
			bb_error_msg("%s%s: %s", date_buf, applet_name, msg);
			free(msg);
#else
			...
			vasprintf(&msg, ctl + 1, va);
#ifdef MY_ABC_HERE
			bb_info_msg("%s%s: %s", date_buf, applet_name, msg);
#else
...
#ifdef MY_ABC_HERE
static char *SYNOReadTZFile(char *szBuf, int BufSize)
{
	int fd = -1;
	ssize_t r;
	size_t todo;
...
static int SYNOTZSetEnv(void)
{
	char szBuf[128];
	int err = -1;

	bzero(szBuf, sizeof(szBuf));
	if (NULL != SYNOReadTZFile(szBuf, sizeof(szBuf))) {
		err = setenv("TZ", szBuf, 1);
	}
	return err;
}
#endif
...
#ifdef SYNO_PPC_QORIQ
static void SYNOScemSetRtcWakeUp(void)
{
	char szCmd[128] = {'\0'};

	snprintf(szCmd, sizeof(szCmd), "/bin/kill -%d `/bin/cat %s` > /dev/null 2>&1", SIG_CROND_EXECUTE, SZF_SCEMD_PIDFILE);
	if (0 > system(szCmd)) {
		syslog(LOG_ERR, "Reset RTC shedule wake up failed.");
	}
}
#endif

#ifdef MY_ABC_HERE
static void SYNOReloadTable(int unused)
{
	unused = 0;
	ReloadCrontab = 1;
}
...
#ifdef MY_ABC_HERE
			/* 
			 * We MUST HUP crond after edit the /etc/crontab
			 * After receive the SIGHUP, we will reload jobs.
			 */
			if (ReloadCrontab == 1) {
#ifdef MY_ABC_HERE
				// When timezone is changed, the SDK will hup crond and reload crontab
				// I will update the envorinment, too.
				SYNOTZSetEnv();
#endif
#ifdef SYNO_PPC_QORIQ
				//When crontab update, reset RTC shedule wake up before deep sleep
				SYNOScemSetRtcWakeUp();
#endif
...
static void SynchronizeFile(const char *fileName)
{
	...
	if (fstat(fileno(parser->fp), &sbuf) == 0 && sbuf.st_uid == DaemonUid) {
		...
		while (1) {
			...
			if (!--maxLines)
				break;
			n = config_read(parser, tokens, TOKEN_NUM, 1, "# \t", PARSE_NORMAL | PARSE_KEEP_COPY);
			if (!n)
				break;
...
#endif
			/* copy command */
			line->cl_Shell = xstrdup(tokens[TOKEN_NUM-1]);
			if (DebugOpt) {
				crondlog(LVL5 " command:%s", tokens[TOKEN_NUM-1]);
			}
			pline = &line->cl_Next;
...
#ifdef MY_ABC_HERE
			if (!strcmp(line->cl_Shell, SZ_SHUTDOWN_CMD) &&
					!access(SZF_ENC_SHARE_LOCK, F_OK)) {
				crondlog(WARN9 "job: %s is cancelled because shared folder settings is being encrypted/decrypted.",
						line->cl_Shell);
				continue;
			}
#endif
...
#ifdef SYNO_PPC_QORIQ
	//After crond execute shedules, reset RTC shedule wake up before deep sleep
	if (blHaveRunJob) {
		SYNOScemSetRtcWakeUp();
	}
#endif
