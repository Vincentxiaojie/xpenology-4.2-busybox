...
#ifdef MY_ABC_HERE
extern int FileFindKey(const char *szFile, const char *szKey);
#define SZF_SIMPLE_SYSLOGD_CONF	"/etc/syslog.deny"
#endif

#ifdef MY_ABC_HERE
#define MAX_SIZE		200*1024 /* 200k */
#define MAX_LOG_ENTRY	2500
#define MAX_OLD_MESSAGE	4
...
static const struct init_globals init_data = {
	...
	.logFileRotate = 1,
#else
	.logFileSize = MAX_SIZE,
	.logFileRotate = MAX_OLD_MESSAGE,
#endif
#endif
#if ENABLE_FEATURE_REMOTE_LOG
	.remoteFD = -1,
...
#ifdef MY_ABC_HERE
static void RotateMessages(void)
{
	int i, pid;
	char szOldFile[PATH_MAX];
	char szNewFile[PATH_MAX];
	struct stat statBuf;
#if ENABLE_FEATURE_ROTATE_LOGFILE
	unsigned long ulMaxOldMessage = G.logFileRotate;
#else
	unsigned long ulMaxOldMessage = MAX_OLD_MESSAGE;
#endif

	for (i = ulMaxOldMessage - 2; i >= 0; i--) {
		snprintf(szOldFile, sizeof(szOldFile), "%s.%d.gz", G.logFilePath, i);
		if (lstat(szOldFile, &statBuf)) {
			// messages.x.gz does not exist, try messages.x
			snprintf(szOldFile, sizeof(szOldFile), "%s.%d", G.logFilePath, i);
			if (lstat(szOldFile, &statBuf)) {
				continue;
			} else {
				snprintf(szNewFile, sizeof(szNewFile), "%s.%d", G.logFilePath, i+1);
				rename(szOldFile, szNewFile);
			}
		} else {
			snprintf(szNewFile, sizeof(szNewFile), "%s.%d.gz", G.logFilePath, i+1);
			rename(szOldFile, szNewFile);
		}
	}
	snprintf(szNewFile, sizeof(szNewFile), "%s.0", G.logFilePath);
	rename(G.logFilePath, szNewFile);
	...
	if (!pid) {
		char szCmd[4096];
		// Child
		snprintf(szOldFile, sizeof(szOldFile), "%s.gz", szNewFile);
		unlink(szOldFile);
		snprintf(szCmd, sizeof(szCmd), "/bin/gzip -f %s", szNewFile);
		system(szCmd);
		unlink(szNewFile);
		exit(0);
	}
...
#define recvbuf (G.recvbuf)
	...
	if (0 == lstat(G.logFilePath, &statBuf)) {
...
int syslogd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (opts & OPT_loglevel) {// -l
		...
		for (
				...
				const
#endif
				CODE *c_pri = prioritynames; c_pri->c_name; c_pri++) {
			if (1 != FileFindKey(SZF_SIMPLE_SYSLOGD_CONF, c_pri->c_name)) {
				G.logLevel |= (1 << c_pri->c_val);
			}
		}
	}
#else
