...
int ps_main(int argc UNUSED_PARAM, char **argv)
{
	...
	opt_complementary = "o::";
#ifdef MY_ABC_HERE
	opt = getopt32(argv, "Zo:aAdefl"IF_FEATURE_SHOW_THREADS("T")"I", &opt_o);
#else
...
#  ifdef MY_ABC_HERE
	opts = getopt32(argv, "I"IF_SELINUX("Z")IF_FEATURE_SHOW_THREADS("T")"w", &w_count);
#  else
...
#  ifdef MY_ABC_HERE
	opts = getopt32(argv, "IZ"IF_FEATURE_SHOW_THREADS("T"));
#  else
...
#ifdef MY_ABC_HERE
	else if (!(psscan_flags & PSSCAN_UIDGID))
		puts("  PID   VSZ STAT COMMAND");
#endif
	else
...
#endif
#ifdef MY_ABC_HERE
		if (!(psscan_flags & PSSCAN_UIDGID)) {
			char buf6[6];
			smart_ulltoa5(p->vsz, buf6, " mgtpezy");
			buf6[5] = '\0';
			len = printf("%5u %s %s  ", p->pid, buf6, p->state);
		} else
#endif
		{
