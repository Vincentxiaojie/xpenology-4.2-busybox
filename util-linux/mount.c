...
#ifdef MY_ABC_HERE
/** 
 * Check if the mount path is used for internal
 * disk.
 * 
 * @return  0	path isn't used for internal disk
 * 			1	path is used for internal disk
 */
static int IsInternalDataVolume(char *szMountPoint)
{
	if ((NULL == szMountPoint) ||
		(0 != strncmp(szMountPoint, "/volume", 7)) ||
		(0 == strncmp(szMountPoint, "/volumeUSB", 10)) ||
		(0 == strncmp(szMountPoint, "/volumeSATA", 11)) ) {
		return 0;
	}
	return 1;
}

static int IsExternalDataVolume(char *szMountPoint)
{
	if ((0 == strncmp(szMountPoint, "/volumeUSB", 10)) ||
		(0 == strncmp(szMountPoint, "/volumeSATA", 11)) ) {
		return 1;
	}
	return 0;
...
static void strAppend(char **szTarget, const char *szSrc)
{
	int StrLen = 0;
	int totalLen = 0;

	if (!*szTarget) {
		*szTarget = strdup(szSrc);
		return;
	}
	StrLen = strlen(*szTarget);
	totalLen = StrLen + strlen(szSrc) + 2;	//add 2 for ',' and '\0'
	*szTarget = xrealloc(*szTarget, totalLen);
	snprintf((*szTarget + StrLen), totalLen - StrLen, ",%s", szSrc);
}

/**
 *  Append the following default mount options "data=writeback",
 *  "oldalloc", and "errors=remount-ro".  If the string already
 *  contained "data=", "errors=", and "orlov", it will not
 *  change the option.
 * 
 *  @param	pszMountOption	The string to be appended
 *                          mount options.
 */
static void extFSMountOptionAppend(char **pszMountOption, char *mntType, int blInternalVolume)
{
	if (NULL == pszMountOption) {
		return;
	}

	if (NULL == *pszMountOption || NULL == strstr(*pszMountOption, "data=")) {
		if (blInternalVolume) {
			strAppend(pszMountOption, "data=writeback");
		} else {
			strAppend(pszMountOption, "data=ordered");
		}
	}
	if (!strcmp(mntType, "ext3")) {
		if (NULL == *pszMountOption || NULL == strstr(*pszMountOption, "errors=")) {
			strAppend(pszMountOption, "errors=remount-ro");
		}
	}
#if SYNO_HAVE_KERNEL_VERSION(2,6,15)
	if (NULL == *pszMountOption || NULL == strstr(*pszMountOption, "orlov")) {
		strAppend(pszMountOption, "oldalloc");
	}
#endif
}
#endif
...
#ifdef MY_ABC_HERE
#define READ_DISK_OFFSET 1024
#define SUPER_BLOCK_BUF_LENGTH   256
#define SYNO_EXT4_FEATURE_INCOMPAT_EXTENTS		0x0040 /* extents support */
#define SYNO_FEATURE_COMPAT_HAS_JOURNAL	    	0x0004 /* has journal */   
#define SYNO_FS_MAGIC_EXT3  0xEF53

/**
 * Reads size character from buff starting at offset
 * returning an integer representation of the data
 * read
 * 
 * @param buf    on disk structure which is little endian
 * @param size   buffer size
 * 
 * @return 
 */
static unsigned int leChar_to_cpu(char *buf, int size)
{
	int i;
	unsigned int res;
	res &= 0x000000;
	for(i=0;i<size;i++){
		res |=((0x000000ff & buf[i]) << (i*8)); 
	}
	return res;
}

/**
 * Copy from libsynosdk/lib/fs_get_ext_type.c, detect file 
 * system is ext3 or ext4. It is considered by extents attribute 
 * of ext4. 
 * 
 * @param szDevPath target file system device path, 
 *                  shouldn't be null
 * 
 * @return success: return FSTYPE_EXT3/FSTYPE_EXT4
 *         failed:  -1
 */
static const char* SYNOGetExtType(const char *szDevPath)
{
	int fd = -1;
	const char *ret = NULL;
	char sb_buf[SUPER_BLOCK_BUF_LENGTH];

	bzero(sb_buf, SUPER_BLOCK_BUF_LENGTH);

	fd = open(szDevPath, O_RDONLY);
	if (0 > fd) {
		bb_error_msg("open failed, msg:%s", strerror(errno));
		goto END_EXT;
	}
	if (lseek(fd, READ_DISK_OFFSET, SEEK_SET) != READ_DISK_OFFSET) { 
		bb_error_msg("lseek failed, msg:%s", strerror(errno));
		goto END_EXT;
	}
	if (read(fd,sb_buf, SUPER_BLOCK_BUF_LENGTH) != SUPER_BLOCK_BUF_LENGTH) { 
		bb_error_msg("read failed, msg:%s", strerror(errno));
		goto END_EXT;
	}
	//  +56 is ext2 magic number position offset.
	if (leChar_to_cpu(sb_buf + 56, 2) != SYNO_FS_MAGIC_EXT3) {
		goto END_EXT;
	}
	//  +96 is ext's incompact flag offset.
	if (leChar_to_cpu(sb_buf + 96, 4) & SYNO_EXT4_FEATURE_INCOMPAT_EXTENTS) {
		ret = "ext4";
		goto END_EXT;
	}
	//  +92 is ext's compact flag offset.
	if (leChar_to_cpu(sb_buf + 92, 4) & SYNO_FEATURE_COMPAT_HAS_JOURNAL) {
		ret = "ext3";
		goto END_EXT;
	}
	ret = "ext2";
...
#ifdef MY_ABC_HERE
		/* 
		 * 1. We won't change mount option forcely if the mount flag 
		 *    MS_RDONLY or MS_REMOUNT is set, fsType isn't ext3/ext4, 
		 *    and the volume isn't internal.
		 * 2. If the journal mode is given by user, we'll not change the
		 *    journal mode.
		 * 3. If error occurs behavior is given by user, we'll not change the
		 *    behavior.
		 */
		if (!(MS_RDONLY & vfsflags) && !(MS_REMOUNT & vfsflags) && (NULL != mp->mnt_type) &&
			(!strcmp(mp->mnt_type, "ext3") || !strcmp(mp->mnt_type, "ext4")) &&
			(NULL != mp->mnt_dir) && (IsInternalDataVolume(mp->mnt_dir))) {
			extFSMountOptionAppend(&filteropts, mp->mnt_type, /* true, internal volume */ 1);
		} else if (
				(NULL != mp->mnt_type) &&
				((0 == strcmp(mp->mnt_type, "ext3")) || (0 == strcmp(mp->mnt_type, "ext4"))) &&
				IsExternalDataVolume(mp->mnt_dir)
				)	{
			extFSMountOptionAppend(&filteropts, mp->mnt_type, /* false, external volume */ 0);
		}
#endif
		errno = 0;
#ifdef MY_ABC_HERE
		if (MS_RDONLY & vfsflags) {
			kill_process_for_umount(mp->mnt_dir);
		}
#endif
...
static int singlemount(struct mntent *mp, int ignore_busy)
{
	...
	if (mp->mnt_type || (vfsflags & (MS_REMOUNT | MS_BIND | MS_MOVE)))
		rc = mount_it_now(mp, vfsflags, filteropts);
#ifdef MY_ABC_HERE
	else if (NULL == mp->mnt_type) {
		if (NULL != (mp->mnt_type = (char*)SYNOGetExtType(mp->mnt_fsname))) {
			rc = mount_it_now(mp, vfsflags, filteropts);
		}
		if (0 > rc) {
			mp->mnt_type = (char*)"squashfs";
			rc = mount_it_now(mp, vfsflags, filteropts);
		}
	}
#endif
								...
								GETMNTENT_BUFSIZE))
			{
				...
				if (!fstype || strcmp(mtpair->mnt_type, fstype) == 0)
				{
					...
					printf("%s on %s type %s (",
						mtpair->mnt_fsname, mtpair->mnt_dir, mtpair->mnt_type);
					if (mtpair->mnt_opts) {
						szAttr = strtok_r(mtpair->mnt_opts, ",", &savePtr);
						if (szAttr && !strcmp(szAttr, "oldalloc")) {
							szAttr = strtok_r(NULL, ",", &savePtr);
						}	
						while(szAttr) {
							printf("%s", szAttr);
							szAttr = strtok_r(NULL, ",", &savePtr);
...
#ifdef MY_ABC_HERE
		/* If there is empty option in /etc/mtab, mnt_opts would be "0". 
		 * It will mount failed with 'Unrecognized mount option "0" or missing value'. */
		if(!strcmp(mtcur->mnt_opts, "0")) {
			mtcur->mnt_opts[0] = '\0';
		}
#endif
