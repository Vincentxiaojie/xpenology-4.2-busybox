...
make_new_session(
...
#ifndef MY_ABC_HERE
	const char *login_argv[2] = {NULL, NULL};
#else
	const char *login_argv[4] = {NULL, NULL, NULL, NULL};
	char szAddr[46];
	...
	if (0 > getpeername(sock, (struct sockaddr *) &peer, &pLen)) {
		syslog(LOG_WARNING, "%s:%s:%d could not getpeername", __FILE__, __func__, __LINE__);
	} else {
		char szCmd[256];
#if ENABLE_FEATURE_IPV6
		if(peer.ss_family == AF_INET6) {
			struct sockaddr_in6 *sinaddr6 = (struct sockaddr_in6 *) &peer;
			char *szptr = NULL;
			inet_ntop(sinaddr6->sin6_family, &(sinaddr6->sin6_addr), szAddr, sizeof(szAddr));
			// v4 Mapped v6 ip
			if(IN6_IS_ADDR_V4MAPPED(&(sinaddr6->sin6_addr))) {
				szptr = strrchr(szAddr, ':');
				if(szptr) {
					++szptr;
					snprintf(szAddr, sizeof(szAddr), "%s", szptr);
				}else {
					syslog(LOG_ERR, "%s:%s:%d unable to get ipv4 address from v4mapped address:%s\n", __FILE__, __func__, __LINE__, szAddr);
				}
			}
		}
		else 
#endif
		{
			struct sockaddr_in *sinaddr = (struct sockaddr_in *) &peer;
			inet_ntop(sinaddr->sin_family, &(sinaddr->sin_addr), szAddr, sizeof(szAddr));
		}
		if (0 == access("/usr/syno/bin/synoautoblock", F_OK)) {
			snprintf(szCmd, sizeof(szCmd), "/usr/syno/bin/synoautoblock --deny \"%s\"", szAddr);
...
#ifdef MY_ABC_HERE
static void handle_sigterm(int sig UNUSED_PARAM)
{
