...
#ifdef MY_ABC_HERE
	/*
	 * IE will stop load page when get error 404 status, it leads error redirect failed,
	 * Set http status as 200(HTTP_OK), let IE redirect to error page
	 */
	if (HTTP_NOT_FOUND == responseNum) {
		responseNum = HTTP_OK;
		for (i = 0; i < ARRAY_SIZE(http_response_type); i++) {
			if (HTTP_OK == http_response_type[i]) {
				responseString = http_response[i].name;
				infoString = http_response[i].info;
				break;
			}
...
#ifdef MY_ABC_HERE
		if (access("/webman/index.cgi"+1, X_OK) == 0) {
			urlp[0] = '\0';
			g_query = urlcopy;
			send_cgi_and_exit("/webman/index.cgi", prequest, length, cookie, content_type);
		}
#else
