...
int login_main(int argc UNUSED_PARAM, char **argv)
{
 ...
 auth_failed:
#ifdef MY_ABC_HERE
		if (opt_host &&
			0 == access("/usr/syno/bin/synoautoblock", F_OK) &&
			0 == access("/usr/syno/bin/synologset1", F_OK)) {
			snprintf(szCmd, sizeof(szCmd),
				"/usr/syno/bin/synologset1 conn warn 0x12A00011 \"%s\" \"%s\" TELENT",
				username, opt_host);
			system(szCmd);

			snprintf(szCmd, sizeof(szCmd), "/usr/syno/bin/synoautoblock --attempt \"%s\" TELENT", opt_host);
	...
	if (opt_host  &&
		...
		0 == access("/usr/syno/bin/synologset1", F_OK)) {
		snprintf(szCmd, sizeof(szCmd),
			"/usr/syno/bin/synologset1 conn info 0x12A00013 \"%s\" \"%s\" TELENT",
			username, opt_host);
		system(szCmd);
		snprintf(szCmd, sizeof(szCmd), "/usr/syno/bin/synoautoblock --reset \"%s\"", opt_host);
		system(szCmd);
	}
#endif
